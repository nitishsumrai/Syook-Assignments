// 4. Short Intro - Some of you might remember spending afternoons playing Street
// Fighter 2 in some Arcade back in the 90s or emulating it nowadays with the numerous
...
function streetFighterSelection(fighters, position, moves) {
    ...
    moves.forEach((e, i) => {
        ...
        switch (e) {
            case 'up':
                r > 0 ? r = r - 1 : r = r;
                ans.push(fighters[r][c])
                break;
            case 'down':
                r < fighters.length - 1 ? r = r + 1 : r = r;
                ans.push(fighters[r][c])
                break;
            case 'left':
                c == 0 ? c = fighters[r].length - 1 : c = c - 1;
                ans.push(fighters[r][c])
                break;
            case 'right':
                c < fighters[r].length - 1 ? c = c + 1 : c = 0;
                ans.push(fighters[r][c])
                break;
        }
    })
